...
func (f *fetcherTester) makeHeaderFetcher(peer string, blocks map[common.Hash]*types.Block, drift time.Duration) headerRequesterFn {
	...
	return func(hash common.Hash, sink chan *zond.Response) (*zond.Request, error) {
...
func (f *fetcherTester) makeBodyFetcher(peer string, blocks map[common.Hash]*types.Block, drift time.Duration) bodyRequesterFn {
	...
	return func(hashes []common.Hash, sink chan *zond.Response) (*zond.Request, error) {
		...
		// Return on a new thread
		bodies := make([]*zond.BlockBody, len(transactions))
		for i, txs := range transactions {
			bodies[i] = &zond.BlockBody{
				Transactions: txs,
				Uncles:       uncles[i],
			}
		}
...
func testConcurrentAnnouncements(t *testing.T, light bool) {
	...
	firstHeaderWrapper := func(hash common.Hash, sink chan *zond.Response) (*zond.Request, error) {
		counter.Add(1)
		return firstHeaderFetcher(hash, sink)
	}
	secondHeaderWrapper := func(hash common.Hash, sink chan *zond.Response) (*zond.Request, error) {
		counter.Add(1)
...
func testPendingDeduplication(t *testing.T, light bool) {
	...
	var counter atomic.Uint32
	headerWrapper := func(hash common.Hash, sink chan *zond.Response) (*zond.Request, error) {
		counter.Add(1)

		// Simulate a long running fetch
		resink := make(chan *zond.Response)
