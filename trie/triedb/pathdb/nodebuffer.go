...
func (b *nodebuffer) revert(db zonddb.KeyValueReader, nodes map[common.Hash]map[string]*trienode.Node) error {
...
func (b *nodebuffer) setSize(size int, db zonddb.KeyValueStore, clean *fastcache.Cache, id uint64) error {
	b.limit = uint64(size)
...
func (b *nodebuffer) flush(db zonddb.KeyValueStore, clean *fastcache.Cache, id uint64, force bool) error {
...
// into clean cache.
func writeNodes(batch zonddb.Batch, nodes map[common.Hash]map[string]*trienode.Node, clean *fastcache.Cache) (total int) {
	for owner, subset := range nodes {
		for path, n := range subset {
			if n.IsDeleted() {
				if owner == (common.Hash{}) {
